...
ggplot(data=state_map_fortified,aes_string(x="Longitude",y="Latitude", group="group")) +
  ...
  theme(axis.title.x = element_text(face="bold", size=28), 
        ...
        legend.title = element_text(size=20, face="bold"),
        plot.title = element_text(hjust = 0.5, face="bold", size=32)) +
  #have to play with this once you set screen size to get legend to display well
  scaleBar(lon = -125.95, lat = 26.8, 
           #legend manipulation
           distanceLon = 500,distanceLat = 50, distanceLegend = 100, dist.unit = "km",
           #arrow manipulation
           arrow.length=500, arrow.distance=200, legend.size=5, arrow.North.size=10, 
           arrow.size=1.25)
