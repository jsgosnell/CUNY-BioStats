...
par(mfcol=c(2,4), oma = c(5,2,2,2), ask=F)
for(j in 1:length(freedom)){
  x=rt(1000, df=freedom[j])
  hist(x, main=paste("t distribution, \n df = ",freedom[j], sep = ""), col = "purple")
  qqnorm(x)
  qqline(x)
}
mtext("1000 Random points drawn from \n various t distributions", outer = T, side = 1, 
      cex = 2, line = 4)
...
for(j in 1:length(freedom)){
  ...
  hist(x, main=paste("chi square distribution, \n df = ",freedom[j], sep = ""), col = "purple")
  qqnorm(x)
  qqline(x)
}
mtext("1000 Random points drawn from \n various chi square distributions", outer = T, side = 1, 
      cex = 2, line = 4)
...
n=c(2,5,25,50)
p=c(.05, .25, .5, .75, .95)
par(mfcol=c(2,2), ask=F, oma=c(5,3,3,3))
for(j in 1:length(p)){
  for (i in 1:length(n)){
    y=0:n[i]
    x=dbinom(y,n[i],p[j])
  ...
  mtext(paste("n draws from Binomial \n distributions with p= ", p[j]), outer = T, side = 1, 
        cex = 2, line = 4)
...
par(mfcol=c(2,3), ask=F, oma=c(5,3,3,3))
for(j in 1:length(u)){
  x=dpois(x=0:(u[j]*3),lambda=u[j])
  plot(0:(u[j]*3),x, type="h", main=paste("mu = ", u[j]), xlab = "")
}
mtext(paste("Poisson distributions \n with mu= ", u[j]), outer = T, side = 1, 
      cex = 2, line = 4)