---
title: "2023 Fall 78002 Midterm answers"
author: "jsg"
...
```{r}
oyster_table <- matrix(c(60,54,62,74,69,81,100,104,95,81,61,77,52,30), nrow=2, byrow = T)
...
```{r}
chisq.test(oyster_table)$expected
```
...
```{r}
growth_lm_ambient <-lm(growth~predator_cue,experiment[experiment$temperature == "ambient",])
plot(growth_lm_ambient)
Anova(growth_lm_ambient, type='III')
```

```{r}
library(multcomp)
summary(glht(growth_lm_ambient, linfct = mcp(predator_cue = "Tukey")))
```

```{r}
growth_lm_scenario1 <-lm(growth~predator_cue,experiment[experiment$temperature == "elevated_scenario1",])
plot(growth_lm_scenario1)
Anova(growth_lm_scenario1, type='III')
```

```{r}
summary(glht(growth_lm_scenario1, linfct = mcp(predator_cue = "Tukey")))
```

```{r}
growth_lm_scenario2 <-lm(growth~predator_cue,experiment[experiment$temperature == "elevated_scenario2",])
plot(growth_lm_scenario2)
Anova(growth_lm_scenario2, type='III')
```
...
experiment_summary$Temperature <- revalue(experiment_summary$temperature, c(ambient = "Ambient", elevated_scenario1 = "IPCC1", elevated_scenario2 = "IPCC2"))

experiment_summary$Predation <- factor(experiment_summary$predator_cue, levels=c("none", "normal", "high"))
