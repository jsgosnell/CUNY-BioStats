...
*Using a bootstrap approach with 10,000 samples (default in the bootstrapjsg function), I found a p-value of 0.0065; this is <.05, so I reject the null 
hypothesis that the 
...
variance,  I found a test statistics of t~7.62~=0.10.  This corresponds to a 
p-value of 0.92. This p-value is >.05, so I fail to reject the null hypothesis that 
...
I found a test statistics of W=13.  This corresponds to a 
p-value of 0.17.  This p-value is >.05, so I fail to reject the null hypothesis that 
the mean clotting times are the same for the two drugs.*
...
I found a test statistics of Z=0.09.  This corresponds to a 
p-value of 0.92.  This p-value is >.05, so I fail to reject the null hypothesis that 
the mean clotting times are the same for the two drugs.*
...
I found a p value of 0.90.  This p-value is >.05, so I fail to reject the null 
hypothesis that 
the mean clotting times are the same for the two drugs.*
...
using a t-test, since the plots suggested a distribution that shoudl approach 
